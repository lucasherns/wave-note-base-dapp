...
const source = await readFile(resolve(process.cwd(), "contracts/WaveNote.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "WaveNote.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["WaveNote.sol"]?.WaveNote;
if (!contract) {
...
console.log(`Deploying WaveNote from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_WAVE_NOTE_CONTRACT_ADDRESS to this contract address in Vercel.");
